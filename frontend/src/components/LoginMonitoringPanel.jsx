...
const LoginMonitoringPanel = ({ history }) => {
  // ✅ Updated Color Logic
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-5 flex flex-col h-full">
      <h3 className="font-semibold text-lg text-gray-800">Login Anomalies</h3>
      <p className="text-sm text-gray-500 mb-4">Suspicious Authentication</p>
      <div className="h-48 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={history}>
            ...
            <Line type="monotone" dataKey="suspicious" stroke="#dc2626" strokeWidth={2} dot={false} name="Suspicious" />
            <Line type="monotone" dataKey="normal" stroke="#10b981" strokeWidth={2} dot={false} name="Normal" />
          </LineChart>
        </ResponsiveContainer>
      </div>
      <div className="mt-6 flex-1 min-h-0">
        <h4 className="text-sm font-semibold text-gray-800 mb-2">Recent Issues</h4>
            ...
            <tbody>
              {[...history].reverse().slice(0, 10).map((row, i) => (
                ...
                <tr key={i} className="bg-white hover:bg-blue-50 border-b last:border-0 animate-fadeIn">
                    ...
                    <td className="py-2 px-3"><span className={`font-bold ${getSeverityColor(row.severity)}`}>{row.severity || "High"}</span></td>
                </tr>
                )
              ))}
